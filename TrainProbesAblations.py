...
def custom_loss(y_true, y_pred, p_x_plus, p_x_minus):
    # Binary cross-entropy component
    bce = BinaryCrossentropy()
    loss_bce = bce(y_true, y_pred)

    avg_p_x_plus = tf.reduce_mean(p_x_plus)
    avg_p_x_minus = tf.reduce_mean(p_x_minus)
    avg_1_p_x_minus = tf.reduce_mean(1 - p_x_minus)
    # Consistency loss component
    loss_consistency = tf.square(avg_p_x_plus - avg_1_p_x_minus)
    # Confidence loss component
    loss_confidence = tf.square(tf.minimum(avg_p_x_plus, avg_p_x_minus))

    # Combine the losses
    final_loss = loss_bce + loss_consistency + loss_confidence
    return final_loss


def define_model(input_dim, add_loss=False):
    """
    ...
    def loss_with_aux_data(y_true, y_pred):
        pos_indices = tf.where(tf.equal(y_true, 1))
        neg_indices = tf.where(tf.equal(y_true, 0))
        # Extract predictions for positive and negative examples
        p_x_plus = tf.gather(y_pred, pos_indices)
        p_x_minus = tf.gather(y_pred, neg_indices)
        return custom_loss(y_true, y_pred, p_x_plus, p_x_minus)
    if add_loss:
        model.compile(optimizer='adam', loss=loss_with_aux_data, metrics=['accuracy'])
    else:
    ...
    for idx in range(len(layers_to_process)):
        ...
        for count, (test_dataset, train_dataset, test_dataset_path) in enumerate(zip(test_datasets, train_datasets, dataset_paths)):
            ...
            for i in range(repeat_each):
                ...
                # Define the model
                model = define_model(train_embeddings.shape[1], args.add_loss)
